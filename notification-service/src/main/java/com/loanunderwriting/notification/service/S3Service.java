...
public class S3Service {
    ...
    public void saveAuditLog(NotificationRecord record) {
        String key = generateS3Key(record.getApplicationId());
    ...
    private String generateS3Key(String applicationId) {
        String date = LocalDateTime.now()
                .format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));
        return String.format("audit-logs/%s/%s.json", date, applicationId);
    }
}